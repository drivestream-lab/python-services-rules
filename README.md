...
|---|---|
| **License** | [MIT](LICENSE) |
| **Version** | see [`VERSION`](VERSION) (currently **0.5.12**) · [CHANGELOG](CHANGELOG.md) |
...
cd .cursor/rules && git checkout v0.5.12 && cd ../..

git add .gitmodules .cursor/rules
git commit -m "Add Python service Cursor rules at .cursor/rules (v0.5.12)"
```
...
(`cookiecutter … --checkout v0.3.5`), then add the rules submodule as above.
...
cd .cursor/rules
git fetch --tags
git checkout v0.5.12    # target version
cd ../..
git add .cursor/rules
git commit -m "Bump Python service rules to v0.5.12"
```
